...
const Index = () => {
  const [allData, setAllData] = useState([]),
    ...
    All = async () => {
      ...
      const response = await fetch('https://localhost:5001/api/municipio/all',
        {
            method: 'GET',
            mode: 'cors',
            headers: {
    ...
    submitForm = async (data) => {
      ...
      var response = null;
      if (!data.id) {
        response = await PostRoute('municipio/store', data)
      } else {
        response = await PostRoute('municipio/update', data)
      }
    ...
    setState = async (id, estado) => {
      ...
      var response = null;
      if (estado === 1) {
        response = await PostRoute('municipio/destroy', { id })
      } else {
        response = await PostRoute('municipio/active', { id })
      }
      response.value === 1 && Refresh()
    },
    SelectRol = async () => {
      const response = await GetRoute('municipio/label')
    ...
    <>
      <div className="row">
        <div className="col  col-lg-11">
          {/* Titulos */}
          <h1>{'Componente de Municipio implementacion CRUD'}</h1>
        </div>
