...
const Index = () => {
  const [allData, setAllData] = useState([]),
    ...
    All = async () => {
      ...
      const response = await fetch('https://localhost:5001/api/genero/all',
        {
            method: 'GET',
            mode: 'cors',
            headers: {
    ...
    submitForm = async (data) => {
      ...
      var response = null;
      if (!data.id) {
        response = await PostRoute('genero/store', data)
      } else {
        response = await PostRoute('genero/update', data)
      }
    ...
    setState = async (id, estado) => {
      ...
      var response = null;
      if (estado === 1) {
        response = await PostRoute('genero/destroy', { id })
      } else {
        response = await PostRoute('genero/active', { id })
      }
      response.value === 1 && Refresh()
    },
    SelectRol = async () => {
    ...
    <>
      <div className="row">
        <div className="col  col-lg-11">
          {/* Titulos */}
          <h1>{'Componente de Género implementacion CRUD'}</h1>
        </div>
