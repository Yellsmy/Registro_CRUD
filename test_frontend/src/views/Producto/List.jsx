...
const List = ({TipoP,One,setState}) => {
    ...
    const columns = [
        ...
        },
        {
            name: 'Existencia',
            center: true,
            cell: (row) => EstadoLista(row.existencia)
        },
        {
            name: 'Acciones',
            center: true,
            cell: (row) =>
                <>
                        <span className='btn btn-sm' onClick={() => One(row, 3)}><Icon.Eye size={14} className="mx-2 text-info" />{'Detalles'}</span>
                        {row.estado === 1 ?
                            <span className='btn  btn-sm' onClick={() => One(row, 2)}><Icon.Edit size={14} className="mx-2 text-primary" /> {"Acutalizar"}</span>
                            : null}
                        <span className='btn btn-sm' onClick={() => setState(row.id_producto, row.existencia)}>
                            {row.existencia === 1 ?
                                <Fragment>
